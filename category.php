...
$cat=$_GET['cat'];   //Searched Query
// Fetch First 3 ADS
function res1($cat,$limit)
{
if($limit!=0)
{
$limit2=$limit-6;
...
$sql="SELECT * FROM ADS WHERE CATEGORY='$cat' ORDER BY aid DESC limit $limit2,$limit";
...
$sql="SELECT * FROM ADS WHERE CATEGORY='$cat' limit $limit2,$limit";
if(!$data=mysql_query($sql))
echo mysql_error();
while($row=mysql_fetch_object($data))
{
...
<a href="category.php?cat=Phone and tablets">Mobile and Tablets</a>  &nbsp;&nbsp;
	...
	<a href="category.php?cat=Others">Others</a> <br \><br \>
		...
		<div class="bottom-grids collections">
				...
				<div align="center">
				<?php
				$sql="SELECT * FROM ADS WHERE CATEGORY='$cat'";
if(!$data=mysql_query($sql))
echo mysql_error();
...
{
echo '
<a href="category.php?cat='.$cat.'&page='.$start.'">'.$start.' </a> &nbsp;    
				 ';
				 $start++;
}
echo '<a href="category.php?cat='.$cat.'&page='.$page.'">'.$page.' </a> &nbsp;    </h4>';
}
echo '</div>';
